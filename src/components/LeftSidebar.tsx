...
const LeftSidebarContent: React.FC<LeftSidebarProps & { handleIconClick: (link: string) => void }> = ({
  ...
  return (
    <>
      ...
      <aside
        className={`flex flex-col items-center py-8 space-y-4 fixed top-0 left-0 h-full z-20 transition-transform duration-300 ease-in-out ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        } ${sidebarOpen ? 'w-full bg-gray-900 bg-opacity-90' : 'w-16 bg-white'} md:w-16 md:bg-white md:bg-opacity-100 md:translate-x-0`}
      >
        ...
        <FaTimes
          onClick={toggleSidebar}
          className={`text-2xl text-white-800 cursor-pointer transition-transform duration-300 md:hidden ${
            sidebarOpen ? 'absolute top-4 right-4' : 'hidden'
          }`}
        />
        {leftIcons.map(({ Icon, link, iconClass }, index) => (
          <button
            ...
            className={`w-8 h-8 rounded-lg flex items-center justify-center hover:bg-purple-100 hover:text-purple-600 transition-colors ${
              iconClass ? iconClass : 'bg-gray-100 text-gray-600'
            }`}
          >
